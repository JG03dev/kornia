...
def inverse_transformation(trans_12: Tensor) -> Tensor:
    ...
    # Fast shape checks, fail before slicing
    shape = trans_12.shape
    if trans_12.dim() == 2:
        if shape != (4, 4):
            raise ValueError(f"Input size must be a Nx4x4 or 4x4. Got {shape}")
        out = torch.eye(4, dtype=trans_12.dtype, device=trans_12.device)
    elif trans_12.dim() == 3:
        if shape[-2:] != (4, 4):
            raise ValueError(f"Input size must be a Nx4x4 or 4x4. Got {shape}")
        out = torch.eye(4, dtype=trans_12.dtype, device=trans_12.device).expand(shape[0], 4, 4).clone()
    else:
        raise ValueError(f"Input size must be a Nx4x4 or 4x4. Got {shape}")

    rmat_12 = trans_12[..., :3, :3]  # always Nx3x3 or 3x3, more efficient slice
    tvec_12 = trans_12[..., :3, 3]  # Nx3 or 3, fetch as vector

    rmat_21 = rmat_12.transpose(-1, -2)  # Nx3x3 or 3x3
    if trans_12.dim() == 2:
        tvec_21 = -rmat_21 @ tvec_12
        out[:3, :3] = rmat_21
        out[:3, 3] = tvec_21
    else:  # batch mode
        tvec_21 = -torch.matmul(rmat_21, tvec_12.unsqueeze(-1)).squeeze(-1)
        out[:, :3, :3] = rmat_21
        out[:, :3, 3] = tvec_21
    return out
