...
def KORNIA_CHECK_DM_DESC(desc1: Tensor, desc2: Tensor, dm: Tensor, raises: bool = True) -> bool:
    ...
    """
    # Cache the dim sizes to avoid repeated method calls
    dm0 = dm.size(0)
    dm1 = dm.size(1)
    d10 = desc1.size(0)
    d20 = desc2.size(0)

    if (dm0 != d10) or (dm1 != d20):
        if raises:
            # Only build the error string if we actually need it
            raise TypeError(
                f"distance matrix shape ({dm0}, {dm1}) is not onsistent with descriptors shape: desc1 ({d10},) desc2 ({d20},)"
            )
        return False
    return True
