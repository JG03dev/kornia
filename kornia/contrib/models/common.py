...
class DropPath(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        # Fast path: no drop or not training
        if self.drop_prob == 0.0 or not self.training:
            return x
        keep_prob = 1.0 - self.drop_prob
        # Precompute shape tuple just once
        shape = (x.shape[0],) + (1,) * (x.ndim - 1)
        # Use inplace random generation for efficiency
        random_tensor = x.new_empty(shape).bernoulli_(keep_prob)
        if self.scale_by_keep and keep_prob > 0.0:  # moved faster condition
            random_tensor.div_(keep_prob)
        return x * random_tensor  # memory efficient broadcasting multiply
